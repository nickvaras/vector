...
HeightMap::HeightMap(ros::NodeHandle node, ros::NodeHandle priv_nh)
{
  ...
  priv_nh.param("min_obstacle_height", min_obstacle_height_, -1000.0);
  priv_nh.param("max_obstacle_height", max_obstacle_height_, 1000.0);
  priv_nh.param("z_offset", z_offset_,0.0);
  
  ROS_INFO_STREAM("height map parameters: "
                  << grid_dim_ << "x" << grid_dim_ << ", "
                  << m_per_cell_ << "m cells, "
                  << height_diff_threshold_ << "m threshold, "
                  << min_obstacle_height_ << "lowest obstacle height, "
                  << max_obstacle_height_ << "highest obstacle height, "
                  << z_offset_ << "z-offset from sensor frame, "
  ...
  for (unsigned i = 0; i < npoints; ++i) {
    ...
    if (x >= 0 && x < grid_dim_ && y >= 0 && y < grid_dim_ && init[x][y]) {
      if ((max[x][y] - min[x][y] > height_diff_threshold_) || 
          ((max[x][y] > min_obstacle_height_) &&
           (max[x][y] < max_obstacle_height_)))
      {
  ...
  for (int x = 0; x < grid_dim_; x++) {
    for (int y = 0; y < grid_dim_; y++) {
      if (num_obs[x][y]>0) {
        ...
        obstacle_cloud_.points[obs_count].z = z_offset_;
        ...
        clear_cloud_.points[empty_count].z = z_offset_;
